...
interface CreateSummaryModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (selectedItems: CapturedItem[], summaryContent: string, summaryType: 'markdown' | 'ai') => void;
...
export default function CreateSummaryModal({
  ...
  const handleSummarySelection = async (summaryContent: string, summaryType: 'markdown' | 'ai') => {
    setIsComparisonModalOpen(false);
    onSubmit(selectedItems, summaryContent, summaryType);
  };
      ...
      const imageUrl = imgContent.dataUrl;
        ...
        return (
            <div className="relative bg-gray-100 rounded">
                ...
                />
                <p className="text-sm text-gray-500 italic hidden text-center py-2">이미지 미리보기 불가</p>
            </div>
        );
    } else if (typeof item.content === 'string') {
  ...
  return (
    <>
      <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 p-4" onClick={onClose}>
        <div className="bg-white rounded-lg p-6 w-full max-w-lg max-h-[80vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
          <h2 className="text-xl font-bold mb-4 flex-shrink-0">요약할 항목 선택</h2>
          <p className="text-sm text-gray-600 mb-4 flex-shrink-0">요약에 포함할 캡처 항목을 선택해주세요.</p>
          <form onSubmit={handleSubmit} className="flex flex-col flex-grow overflow-hidden">
            <div className="mb-4 flex-grow overflow-y-auto overflow-x-hidden space-y-3 pr-2">
              {items.map(item => (
                <div key={item.id} className={`flex items-start p-3 border rounded ${selectedItemIds.has(item.id) ? 'border-blue-300 bg-blue-50' : 'border-gray-200 bg-white'}`}>
                  <input
                    type="checkbox"
                  ...
                  <label htmlFor={`item-${item.id}`} className="flex-1 cursor-pointer">
                    <span className={`font-medium ${selectedItemIds.has(item.id) ? 'text-blue-800' : 'text-gray-800'}`}>
